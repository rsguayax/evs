...
<html>
<head>
<title>Tipo de Evento de evaluaci&oacute;n</title>
...
<script type="text/javascript">
	$(function() {
		...
		$('#tabEvaluationEventType').addClass('active');
...
<body>
	<div class="page-header">
 		<h3>Edici&oacute;n del tipo de evento de evaluaci&oacute;n ${evaluationEvent.name}
 			<small>
			        ...
			        <div class="tab-pane active">
			        	<sf:form modelAttribute="evaluationEvent">
			        		...
			        		<sf:label path="evaluationEventTypes">Tipo de evento de evaluaci&oacute;n</sf:label>
								<sf:select path="evaluationEventTypes" items="${evaluationEventTypes}" itemLabel="name" itemValue="idAsString" class="form-control" id="evaluation-event-types-list">
								</sf:select> <sf:errors path="evaluationEventTypes" cssClass="error" />
							</div>
							...
							<div class="form-group">
								...
								<sf:input path="startDate" id="startDate" class="form-control" type="datetime" /> <sf:errors path="startDate" cssClass="error" />
							</div>
							<div class="form-group">
								<sf:label path="endDate">Fecha de fin</sf:label>
								<sf:input path="endDate" id="endDate" class="form-control" type="datetime" /> <sf:errors path="endDate" cssClass="error" />
							</div>
							<div class="form-group">
								<sf:label path="testDuration">Duraci&oacute;n del examen (minutos)</sf:label>
								<sf:input path="testDuration" id="testDuration" class="form-control" type="number" /> <sf:errors path="testDuration" cssClass="error" />
							</div>
							<div class="form-group">
								<sf:label path="comment">Observaci&oacute;n</sf:label>
								<sf:textarea path="comment" id="comment" class="form-control" /> <sf:errors path="comment" cssClass="error" />
								<sf:hidden path="id" readonly="true" />
							</div>
							<div class="form-group">
								<sf:label path="evaluationTypes">Etiquetas del Evento</sf:label>
								<sf:select path="evaluationTypes"  multiple="true" items="${evaluationTypes}" itemLabel="name" itemValue="idAsString" class="form-control" id="evaluation-types-list">
								</sf:select> <sf:errors path="evaluationTypes" cssClass="error" />
							</div>
