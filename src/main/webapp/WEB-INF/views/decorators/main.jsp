...
<body role="document">
          ...
          <sec:authorize access="isAuthenticated()">
	          <div class="navbar-collapse collapse" ng-controller="auxCtrl">
	            <ul class="nav navbar-nav">
	            	<sec:authorize access="@evsSecurityHandler.hasMProfile('administrator', 'logistics_manager', 'evaluation_center_supporter')">
	            		<li><a href="<c:url value="/evaluationevent"/>" data-path="evaluationevent">Eventos de evaluaci&oacute;n</a></li>
	            		<li><a href="<c:url value="/evaluationcenter"/>" data-path="evaluationcenter">Centros de evaluaci&oacute;n</a></li>
	            	</sec:authorize>	
	            	<sec:authorize access="@evsSecurityHandler.hasMProfile('administrator', 'logistics_manager', 'university_center_supporter')">           		
	           			<li><a href="<c:url value="/students"/>" data-path="students">Estudiantes</a></li>
	           		</sec:authorize>
	           		<sec:authorize access="@evsSecurityHandler.hasMProfile('administrator', 'logistics_manager', 'university_center_supporter')">           		
	           			<li><a href="<c:url value="/user"/>" data-path="user">Usuarios</a></li>
	           		</sec:authorize>
	           		<sec:authorize access="@evsSecurityHandler.hasMProfile('administrator', 'logistics_manager', 'evaluation_center_supporter')"> 	
		               	<li class="dropdown">
		                	<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Gesti&oacute;n acad&eacute;mica <span class="caret"></span></a>
			                <ul class="dropdown-menu">
			                  <sec:authorize access="@evsSecurityHandler.hasMProfile('administrator', 'logistics_manager', 'banks_manager')">
			                  	<li><a href="<c:url value="/bank"/>">Bancos de preguntas</a></li>
			                  </sec:authorize>
			                  <sec:authorize access="@evsSecurityHandler.hasMProfile('administrator', 'logistics_manager')">
		                  		  <li><a href="<c:url value="/evaluationeventtype"/>">Tipos eventos de evaluación</a></li>
				                  <li><a href="<c:url value="/matter"/>">Asignaturas</a></li>
				                  <li><a href="<c:url value="/reports"/>">Informes</a></li>
				                  <li><a href="<c:url value="/server"/>">Servidores</a></li>
				                  <li><a href="<c:url value="/degree"/>">Titulaciones</a></li>
			                  </sec:authorize>	                 
			                  <sec:authorize access="@evsSecurityHandler.hasMProfile('administrator', 'logistics_manager', 'evaluation_center_supporter')"> 
				                  <li role="separator" class="divider"></li>
				                  <li class="dropdown-header">Procesos</li>		                  
				                  <li><a href="#" ng-click="updateCenters()">Cargar centros educativos</a></li>
				                  <li><a href="#" ng-click="updateAuxData()">Cargar datos</a></li>
			                  </sec:authorize>
			                </ul>
		              	</li>
	              	</sec:authorize>
    ...
    <div class="container-fluid"  class="ng-cloak"  ng-controller="alertCtrl" ng-init="init('${successMessage}', '${warningMessage}', '${dangerMessage}', '${infoMessage}');">
	 	<div class="jumbotron">
			<%--
	...
	<script type="text/javascript" src="<c:url value="/web-resources/js/ui-calendar/gcal.js" />"></script>
 	...
 	<script src="<c:url value="/web-resources/js/controller/alert-ctrl.js" />"></script>
 	
 	<script src="<c:url value="/web-resources/js/angular-animate.min.js" />"></script>
